...
void MBaseSolverV3::solve(InputMatrix& input) {
...
void MBaseSolverV3::print(std::ostream& stream, const bool *pBoolean, int size) const{
...
bool* MBaseSolverV3::generateCandidate(int size, const bool* father, int indexToAdd){
...
int MBaseSolverV3::check(const bool *pBoolean, int toAdd, InputMatrix& inputMatrix) {
...
int MBaseSolverV3::getSuccessor(int val, int other) const {
    return val+1< other?
        val+1:
        other;
}

int MBaseSolverV3::getMax(const bool* element, int size) const {
    int max=size;
    do{
        max--;
    }
...
bool* MBaseSolverV3::getRepresentativeVector(const bool *pBoolean, InputMatrix &inputMatrix) {
    int size=inputMatrix.getRowLength();
...
bool* MBaseSolverV3::getRepresentativeVector(int index, InputMatrix &inputMatrix) {
...
int MBaseSolverV3::evaluateTruthMap(bool contains0, bool contains1, bool contains2, bool contains3) const {
    return truthMap[contains0*8+contains1*4+contains2*2+contains3];
}
